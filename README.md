...
<sub>数据每 6 小时通过 bStats API 自动更新 · 最近更新: 2026-03-08 12:45 UTC</sub>
<!-- BSTATS-STATS-END -->
...
| **游戏影响** | Pufferfish DAB 会修改远处 AI 行为 | **默认优先保持原版；保护项按需显式开启** |
...
TreeCore 内置了一个 **11 页签的 Web 管理面板**（当前资源版本 `v4.7`）。
...
| 页面 | 功能 |
|---|---|
| **仪表盘** | TPS / MSPT / CPU / 内存实时监控，性能趋势图表，JVM 内存条、CPU 负载条、峰值对比、红石热点与热点回放 |
| **控制台** | 远程执行命令，实时日志流（INFO/WARN/ERROR 过滤 + 关键字过滤），自动滚动 |
| **诊断工具** | JVM 内存快照、主线程卡顿检测、线程转储、GC 操作 |
| **核心配置** | 可视化编辑 TreeCore 参数，配置搜索过滤，分组折叠，高级模式（原始 JSON） |
| **生电优化** | 红石热点限流 / 回放开关、假人系统管理、TNT / 地毯复制开关 |
| **模型管理** | 拖拽上传模型与纹理文件，模型列表查看 / 删除，配合资源包流程使用 |
| **资源包** | 拖拽上传本地 ZIP、重载 SHA-1、本地托管 / 外部 URL 切换、远程下发在线玩家、CraftEngine 同步桥 |
| **粒子特效** | 拖拽式画布编辑器，自定义特效保存 / 读取 / 删除，预设特效（blizzard / nova / helix / ring） |
| **集成运维** | CraftEngine / Multiverse / ItemsAdder 状态卡片，世界创建 / 加载 / 卸载，筛选与批量卸载，一键更新任务列表 |
| **IA 编辑器** | ItemsAdder 菜单编辑、槽位配置、菜单预览、已配置槽位列表 |
...
│
├─ Web Panel (v4.7)                    # Web 管理面板（11 页）
│   ├─ Dashboard / Console             # 仪表盘 / 控制台
│   ├─ Diagnostics / Config            # 诊断 / 配置
│   ├─ Redstone / Models               # 生电 / 模型
│   ├─ Resource Pack / Effects         # 资源包 / 粒子特效
│   ├─ Integrations / IA Editor        # 集成运维 / IA 编辑器
│   └─ Backups                         # 备份管理
│
├─ Game Features                       # 游戏功能
